"""
Mybatis Context Generator 검증 스크립트

실제 타겟 프로젝트의 table_access_info.json을 읽어서
Context Generator가 어떤 파일들을 context로 묶어서 LLM에게 전달하는지 검증합니다.

지원하는 sql_wrapping_type:
    - mybatis: 일반 Spring MVC + MyBatis 프로젝트

사용법:
    python examples/run_mybatis_context_generator.py --config /path/to/config.json
...
def setup_logging(verbose: bool = False):
    ...
    return logging.getLogger("run_mybatis_context_generator")
...
def create_context_generator(
    ...
    """
    return MybatisContextGenerator(config, mock_code_generator)
...
def extract_layer_from_path(file_path: str) -> str:
    ...
    Returns:
        str: 레이어 힌트 (controller, service, repository 등)
    """
    path_lower = file_path.lower()
    filename_lower = Path(file_path).name.lower()

    # Controller 레이어
    if "/controller/" in path_lower or "\\controller\\" in path_lower:
        return "Controller"
    if filename_lower.endswith("controller.java"):
        return "Controller"

    # Service 레이어
    if "/service/" in path_lower or "\\service\\" in path_lower:
        if filename_lower.endswith("impl.java"):
            return "ServiceImpl"
        return "Service"
    if filename_lower.endswith("serviceimpl.java"):
        return "ServiceImpl"
    if filename_lower.endswith("service.java"):
        return "Service"

    # Repository/DAO 레이어
    if "/repository/" in path_lower or "\\repository\\" in path_lower:
        return "Repository"
    if "/dao/" in path_lower or "\\dao\\" in path_lower:
        return "DAO"
    if filename_lower.endswith("repository.java"):
        return "Repository"
    if filename_lower.endswith("dao.java"):
        return "DAO"

    # VO 파일
    if filename_lower.endswith("vo.java"):
        return "VO"
    if "/vo/" in path_lower or "\\vo\\" in path_lower:
        return "VO"

    # Mapper 파일
    if filename_lower.endswith("mapper.java") or filename_lower.endswith("mapper.xml"):
        return "Mapper"
...
def main():
    parser = argparse.ArgumentParser(
        description="Mybatis Context Generator 검증 스크립트"
    )
    parser.add_argument(
        "--config",
        type=str,
    ...
    supported_types = ["mybatis"]
    if config.sql_wrapping_type not in supported_types:
        logger.warning(
            f"이 스크립트는 주로 {supported_types}를 대상으로 합니다. "

                    ...
                    if start_idx != -1:
                        ...
                        if brace_start != -1:
                            ...
                            for i in range(brace_start, len(content)):
                                ...
                                raise CodePatcherError("JSON 파싱 실패: 올바른 JSON 형식이 아닙니다.")
                        else:
                            raise CodePatcherError("JSON 파싱 실패: modifications를 찾을 수 없습니다.")
                    else:
                        raise CodePatcherError("JSON 파싱 실패: modifications 키를 찾을 수 없습니다.")
                else:
                    raise CodePatcherError("JSON 파싱 실패: 올바른 JSON 형식이 아닙니다.")
    ...
    def apply_patch(
        ...
        try:
            ...
            if not file_path.is_absolute():
                ...
                logger.warning(f"상대 경로가 전달되었습니다: {file_path}. 절대 경로로 변환합니다.")
    ...
    def apply_patch_using_difflib(
        ...
        try:
            ...
            while i < len(diff_lines):
                ...
                if line.startswith("@@"):
                    ...
                    if match:
                        ...
                        while len(modified_lines) < new_start and current_old_line < len(original_lines):
            ...
            if ratio == 1.0:
                ...
                generated_diff = list(difflib.unified_diff(
                    original_lines,
                    modified_lines,
                ...
                if ratio < 0.3:
                    logger.warning(f"파일 변경 비율이 매우 큽니다 ({ratio:.2%}). 원본 diff 파싱을 확인하세요.")
            ...
            logger.info(f"파일 수정 완료 (difflib 검증 사용): {file_path}, 변경 비율: {ratio:.2%}")
            return True, None

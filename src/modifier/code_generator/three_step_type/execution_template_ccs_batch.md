# Code Modification Execution (Phase 3) - CCS Batch Version
...
**CORRECT output (preserves comment, adds encryption/decryption):**
```java
while (reader.next()) {
    CustVO vo = reader.read();
...
======FILE_1======
======MODIFIED_CODE======
package sli.ccs.cr.cmpgnmng.bat;
...
## Batch Program Modification Patterns

### Pattern 1: DECRYPT after ItemReader.read()
...
### Pattern 2: ENCRYPT before ItemWriter.write()
...
### Pattern 4: Data Transfer between VOs

**Original:**
```java
SourceVO src = srcReader.read();
TargetVO tgt = new TargetVO();
tgt.setCustNm(src.getCustNm());
tgtWriter.write(tgt);
```
...
SourceVO src = srcReader.read();
// Decrypt source data
src.setCustNm(SliEncryptionUtil.decrypt(0, SliEncryptionConstants.Policy.NAME, src.getCustNm()));

TargetVO tgt = new TargetVO();
tgt.setCustNm(src.getCustNm());  // Already decrypted

// Encrypt target data before write
...
6. **Follow insertion_point exactly**: Insert encryption/decryption code at the exact location specified
7. **DECRYPT after read()**: Insert decryption immediately after `ItemReader.read()` returns
8. **ENCRYPT before write()**: Insert encryption immediately before `ItemWriter.write()` call
9. **Preserve all existing code**: Do not remove or modify any existing code other than the encryption/decryption additions
10. **No explanations**: Do not add any explanations or reasoning. Just output the code.

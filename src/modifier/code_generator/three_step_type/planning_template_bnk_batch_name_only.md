...
3. **Check `crypto_fields`** for NAME columns only - if empty → `action: "SKIP"` with `file_name: ""` and `target_method: ""`
...
- **SKIP format**: When action is SKIP, use `file_name: ""` and `target_method: ""`

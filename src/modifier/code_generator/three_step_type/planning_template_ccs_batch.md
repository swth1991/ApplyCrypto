...
5. **If `crypto_fields` is empty** → output `action: "SKIP"` with appropriate reason
...
| `file_name`         | BAT.java file name to modify                             | `CmpgnCstmrRegBAT.java`                                       |
| `target_method`     | Method name to modify (usually `execute`)                | `execute`, `process`                                          |
...
```json
{
  ...
  "modification_instructions": [
    {
      "flow_id": "FLOW_001",
      "file_name": "StatusUpdateBAT.java",
      "target_method": "execute",
      "action": "SKIP",
...
3. **Check `crypto_fields`** - if empty → `action: "SKIP"`

...
class AnyframeRps2ContextGenerator(BaseContextGenerator):
    ...
    def generate(
                ...
                for cs in sq.get("call_stacks", []):
                    ...
                    for entry in cs:
                        ...
                        if class_name in biz_name_to_path:
                            anchor_pairs_to_biz_names[anchor_pair].add(class_name)
                        elif (
                            class_name in interface_name_to_path
                            and class_name != svc_name
                        ):
                            anchor_pairs_to_svc_names[anchor_pair].add(class_name)
                        elif (
        ...
        for anchor_pair, biz_names in anchor_pairs_to_biz_names.items():
            ...
            svc_names_from_cs = anchor_pairs_to_svc_names.get(anchor_pair, set())

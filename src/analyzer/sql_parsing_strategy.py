...
class MyBatisStrategy(SQLParsingStrategy):
    ...
    def extract_table_names(self, sql_query: str) -> Set[str]:
        ...
        insert_pattern = r'\bINSERT\s+INTO\s+([a-zA-Z_][a-zA-Z0-9_]*(?:\.[a-zA-Z_][a-zA-Z0-9_]*)?)\b'
        ...
        # UPDATE 절에서 테이블명 추출
        update_pattern = r'\bUPDATE\s+([a-zA-Z_][a-zA-Z0-9_]*(?:\.[a-zA-Z_][a-zA-Z0-9_]*)?)\b'
        ...
        delete_pattern = r'\bDELETE\s+FROM\s+([a-zA-Z_][a-zA-Z0-9_]*(?:\.[a-zA-Z_][a-zA-Z0-9_]*)?)\b'
    ...
    def extract_column_names(self, sql_query: str, table_name: str) -> Set[str]:
        ...
        alias_pattern = rf'\bFROM\s+{re.escape(table_name)}\s+([a-zA-Z_][a-zA-Z0-9_]*)\b'
        ...
        if select_match:
            ...
            for match in column_matches:
                if '.' in match:
                    ...
                    parts = match.split('.')
                    if len(parts) == 2:
                        if parts[0].upper() == table_name.upper() or parts[0] == table_alias:
        ...
        update_pattern = rf'\bUPDATE\s+{re.escape(table_name)}\s+SET\s+(.*?)(?:\s+WHERE|\s*$)'

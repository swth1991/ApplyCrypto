...
def test_analyze_column_level(db_analyzer, sample_source_files):
    ...
    table_access_info_list = db_analyzer._analyze_column_level(file_table_map, layer_files)
    
    assert len(table_access_info_list) > 0
    
    # USERS 테이블 정보 확인
    users_info = next((info for info in table_access_info_list if info.table_name == "USERS"), None)
    if users_info:

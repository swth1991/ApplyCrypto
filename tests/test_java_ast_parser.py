...
def test_extract_method_info(java_parser, sample_java_file):
    ...
    # 메서드 개수 확인
    assert (
        len(class_info.methods) >= 5
    )  # getUser, createUser, deleteUser, validateId, validateUser
...
def test_extract_field_info(java_parser, sample_java_file):
    ...
    # userService 필드 확인
    user_service_field = next(
        (f for f in class_info.fields if f["name"] == "userService"), None
    )
...
def test_extract_method_calls(java_parser, sample_java_file):
    ...
    assert "validateId" in getUser_method.method_calls or any(
        "validateId" in call for call in getUser_method.method_calls
    )
...
def test_extract_call_relations(java_parser, sample_java_file):
    ...
    assert any(
        "getUser" in rel.caller and "validateId" in rel.callee for rel in relations
    )

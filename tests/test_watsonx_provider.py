...
class TestWatsonXAIProvider:
    ...
    def provider_config(self):
        ...
        if not creds:
            pytest.skip(
                "WatsonX.AI credential이 환경변수에 설정되지 않았습니다. "
    ...
    def test_provider_initialization_success(self, provider_config):
        ...
        assert provider.api_url == provider_config.get(
            "api_url", "https://us-south.ml.cloud.ibm.com"
        )
        assert provider.model_id == provider_config.get(
    ...
    def test_call_success(self, provider_config):
        ...
        assert result["model"] == provider_config.get(
            "model_id", "ibm/granite-13b-chat-v2"
        )
    ...
    def test_validate_response_valid(self, provider_config):
        ...
        invalid_response2 = {"content": "", "tokens_used": 100, "model": "test-model"}
    ...
    def test_provider_initialization_without_credentials(self):
        ...
        with pytest.raises(Exception):
            WatsonXAIProvider(
                api_key="invalid-api-key", project_id="invalid-project-id"
            )

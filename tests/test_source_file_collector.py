...
def temp_project_dir():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        # Java 파일 생성
        (project_path / "src" / "main" / "java" / "Main.java").write_text(
            "public class Main {}"
        )
        (project_path / "src" / "main" / "java" / "Service.java").write_text(
            "public class Service {}"
        )

        # XML 파일 생성
        (project_path / "src" / "main" / "resources" / "mapper.xml").write_text(
            "<mapper></mapper>"
        )
...
def config_file_with_excludes(temp_project_dir):
    ...
    (temp_project_dir / "test" / "TestService.java").write_text(
        "public class TestService {}"
    )
    (temp_project_dir / "test" / "ServiceTest.java").write_text(
        "public class ServiceTest {}"
    )
    (temp_project_dir / "generated" / "Generated.java").write_text(
        "public class Generated {}"
    )
    (temp_project_dir / "src" / "main" / "java" / "MainTest.java").write_text(
...
def test_cross_platform_path_compatibility(collector):
    ...
    for file in files:
        ...
        # 상대 경로가 올바른지 확인
        assert not file.relative_path.is_absolute() or str(
            file.relative_path
        ).startswith("/")

...
from analyzer.sql_extractors.mybatis_sql_extractor import MyBatisSQLExtractor
from analyzer.sql_extractors.jdbc_sql_extractor import JDBCSQLExtractor
from analyzer.sql_extractors.jpa_sql_extractor import JPASQLExtractor
...
def test_create_extractor_case_insensitive(sample_config, mock_xml_parser):
    """대소문자 구분 없이 Extractor 생성 (내부에서 .lower() 처리)"""
    sample_config.sql_wrapping_type = "mybatis"
...
def test_create_unsupported_sql_wrapping_type(sample_config):
    ...
    mock_config = Mock()
    mock_config.sql_wrapping_type = "unsupported"
    mock_config.framework_type = "SpringMVC"
    with pytest.raises(ValueError, match="지원하지 않는 sql_wrapping_type"):

#!/usr/bin/env python3
"""
Modifier Component 다이어그램 렌더링 스크립트
"""
...
try:
    ...
    print("오류: plantuml 모듈이 설치되어 있지 않습니다.")
    print("설치: pip install plantuml")
    exit(1)

def render_plantuml(puml_file: Path, output_file: Path):
    """PlantUML 파일을 PNG로 렌더링"""
    try:
        ...
        print(f"완료: {output_file.name} ({file_size:,} bytes)")
        return True
...
if __name__ == "__main__":
    ...
    puml_file = script_dir / "modifier_component_diagram.puml"
    output_file = script_dir / "modifier_component_diagram.png"
    
    if puml_file.exists():
        render_plantuml(puml_file, output_file)
    else:
